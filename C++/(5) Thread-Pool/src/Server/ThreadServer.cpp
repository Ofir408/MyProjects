...
void* ThreadServer::gamesRunner(void* input) {
	...
	*gameDescription.shouldContinue = false;
	...
	gameDescription.server->getThreadPool()->terminate(); // terminate;
	ThreadServer::exitFromAllTheGames(temp,  gameDescription.server); // check if we need to delete this line.
	//delete gameDescription.server;
