...
void* ThreadStarter::workWithClients(void* des) {
	...
	gameDescription.isInGame = gameDes->isInGame;
	...
	while (true) {
		try {
			...
			gameDescription.firstClientSocket =
					gameDescription.secondClientSocket;
...
void* ThreadStarter::readAndRunCommand(void* des) {
	...
	bool intoGame = gameDes->isInGame;
	Server* server = gameDes->server;
	...
	CommandsManager cmToReturn();

	// add the current command to the thread pool if it's not join request
	// since join need very large part from our thread pool since it runs the game
	// until it will be ended.

	struct argsToExcuteCommand* dataForExcuteCommand =
			new argsToExcuteCommand();
	dataForExcuteCommand->args = vec;
	dataForExcuteCommand->command = args[0];
	dataForExcuteCommand->gameManager = gameManager;
	dataForExcuteCommand->commandsMap = commandsMap;
	// now send this struct as void* as needed.
	if (args[0] == "join" || args[0] == "play") {

		pthread_t thread;
		// add the needed in the next line:
		pthread_create(&thread, NULL, CommandsManager::executeCommand,
				(void*) dataForExcuteCommand);

//CommandsManager::executeCommand((void*) dataForExcuteCommand);
		return (void*) 1; // since we dont want to add this task.
	}
	Task* newTask = new Task(CommandsManager::executeCommand,
			(void*) dataForExcuteCommand);
	if (!intoGame) {
