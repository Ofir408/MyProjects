...
public:
	...
	/**
	 * as its name, gets an integer and return the string of him.
	 */
	 ...
	 * row is the row where the sign has been put in
	 * col is the column where the sign has been put in
	 */
	...
	int makeFirstMessage(string command, string gameName);
	void setTurnByCommand(string command);
	string askForGameList();
