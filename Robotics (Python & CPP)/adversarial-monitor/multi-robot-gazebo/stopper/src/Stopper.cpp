...
Stopper::Stopper(string robot_name, double forward_speed, double rotation_speed, double min_angle, double max_angle, double step_rotation_constant)
{
    ...
    commandPub = nh.advertise<geometry_msgs::Twist>(robotName + "/cmd_vel_mux/input/navi", 10);

    // Subscribe to the simulated robot's laser scan topic
    laserSub = nh.subscribe(robotName + "/scan", 1, &Stopper::scanCallback, this);
    subToThreatsEvents = nh.subscribe(robotName + "/robot_caught_in_threat", 10, &Stopper::stopRobot, this);
...
void Stopper::stopRobot(const std_msgs::Bool::ConstPtr& msg) {
     bool shouldStop = msg->data;
     if (shouldStop) {
		ROS_INFO("%s", "Stop the movment of the robot...");
