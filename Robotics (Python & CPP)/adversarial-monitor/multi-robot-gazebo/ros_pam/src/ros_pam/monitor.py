...
class Tracker(object):
    ...
    def tracker_callback(self, odom_message):
        rospy.logdebug("ON tracker_callback")
        grid_col, grid_row = self.indices_from_position(odom_message.pose.pose.position)
        occupied_cell = (grid_row * self.threat_map.info.width) + grid_col
    ...
    def indices_from_position(self, position):
        print "position is: " + str((position.x, position.y))
        ret = (-1, -1)
        grid_resolution = self.threat_map.info.resolution
        # position can be negative, so the column/row is calculated by devising position by resolution
        # and shifting that by half of the grid's width/height accordingly.
        print "grid_resolution is: " + str(grid_resolution)
        grid_col = int(position.x * 3) + 30
        grid_row = int(position.y * 3) + 30
        print "current cell is: " + str((grid_row, grid_col))
        #grid_row = int(position.y / grid_resolution) + (self.threat_map.info.height / 2)
        #grid_col = int(position.x / grid_resolution) + (self.threat_map.info.width / 2)
...
def dynamic_reconfigure_callback(config, _):
    ...
    Tracker.NORM_FACTOR = float(config["probability_normalization_factor"])
    return config
