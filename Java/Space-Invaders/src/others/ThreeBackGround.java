...
/**
 * ThreeBackGround Class - implements Sprite. The three background in our game -
 * "Green 3"
 *
...
public class ThreeBackGround implements Sprite {
    ...
    private void drawRectangles(DrawSurface d) {
        ...
        while (i < 6) {
            for (int secondCounter = 0; secondCounter < number; secondCounter += 1) {
                d.fillRectangle(57 + 17 * (i - 1), 490 + 30 * (secondCounter - 1), 10, 25);
            }
            i += 1;
        }
    }
    ...
    public List<Block> getBlocks() {
        ...
        Color c = Color.GRAY;
        Block[] slides = {new Block(new Rectangle(new Point(0, 40), 800, 20), 1, c, stroke),
                new Block(new Rectangle(new Point(0, 600), 20, 600), 1, c, stroke),
                new Block(new Rectangle(new Point(780, 600), 20, 600), 1, c, stroke),
                new Block(new Rectangle(new Point(0, 620), 600, 20), 1, c, stroke)};
        ...
        for (int t = 40; t < 40 * 6; t += 40) {
            for (int i = 0; i < 500 - t; i += 50) {
                ...
                Block b1 = new Block(new Rectangle(new Point(730 - i, 140 + t / 2), 50, 20),
                        hits, c, stroke);
                blocksList.add(b1);
            }
            hits = 1;
