...
public class Paddle implements Sprite, Collidable, Shooter {
    /**
    ...
    private Color color;
    /**
     * how much time passed (because after 0.35s = 21 DrawOn of paddle.
     * can shot more one ball)
     */
    private int counterFromLastShot;
    private GameLevel game;
    private boolean paddleWasHitted = false;
    ...
    public Velocity hit(final Ball ball, final Point collisionPoint, final Velocity currentVelocity) {
        ...
        if (ball.getIsBallFromAlien()) {
            this.game.getNumberOfLivesCounter().decreaseOne();
            ball.setCenterPoint(new Point(-30, -30)); // out of screen.
            this.game.setToDefaultCollection();
            this.paddleWasHitted = true;

            // ball.removeFromGame(g);
            // decrease one life because hit the player.
    ...
    /**
     * @return true if paddle was hitted
     */
    public boolean ifPaddleHitted() {
        return this.paddleWasHitted;
    }

    /**
     * @param setTo - set this.paddleWasHitted  = setTo.
     */
    public void setPaddleHittedTo(boolean setTo) {
        this.paddleWasHitted = setTo;
    }
    ...
    @Override
    // ensure that this function is called just if the player press about "space"
    // And 0.35s passed from the last shot - it means every 21 doOneFrame because each is 1/60.
    public void shot(GameLevel gameLevel) {
        double x1 = this.rec.getUpperLeft().getX();
        double x2 = x1 + this.rec.getWidth();
        double y = this.rec.getUpperLeft().getY() + 3;
        // start point of ball.
        Point start = new Point((x1 + x2) / 2.0, y);
        Ball ballToShot = new Ball(start, 3, Color.WHITE);
        ballToShot.setVelocity(0, -6);
        ballToShot.setGameEnvironment(gameLevel.getGameEnvironment());
        ballToShot.addToGame(gameLevel);
        gameLevel.addBall(ballToShot);

    }

    /**
     * @return true if can shot more one ball from the paddle.
     * Otherwise, return false.
     */
    public boolean canShotOneMore() {
        if (this.counterFromLastShot >= 21) {
            this.counterFromLastShot = 0; // to continue check.
            return true;
        }
        return false;
    }
