...
public class Block implements Collidable, Sprite, HitNotifier {
     ...
     * @param stroke        - the stroke of this block.
     */
    public Block(Rectangle r, int maxHitsNumber, Image img, Color stroke) {
        this.rec = r;
        this.hitPoints = maxHitsNumber;
        this.image = img;
        this.blockStrokeColor = stroke;
    ...
    public Block(Rectangle r, int maxHitsNumber, File fileImageForBlock, Color stroke) {
        ...
        try {
            ...
            System.out
                    .println("IO exception in block constructor," + " can't load the image from your file path(***) ");
        }
    }
    ...
    public Block(Rectangle r, int maxHitsNumber, Map<Integer, File> filesMap, Color stroke) {
        ...
        for (int i = 0; i < filesMap.size(); i += 1) {
            try {
                ...
                System.out.println("IO exception in block constructor," + " can't load the image " + (i + 1)
    ...
    public Block(Rectangle r, int maxHitsNumber, Color stroke, Map<Integer, Color> colorsMap) {
        ...
        for (int i = 1; i < colorsMap.size() + 1; i += 1) {
            try {
                ...
                System.out
                        .println("Exception in block constructor," + " can't load the color " + i + " from the input ");
            }
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    System.out.println("IllegalArgumentException exception in block constructor,"
                            + " can't load the image " + i + " from your file path(***)");
                }
            } catch (IOException e) {
                // maybe it's color, continue check
                try {
                    ...
                    System.out.println("IO exception in block constructor," + " can't load the image " + i
                            + " from your file path(**");
                }
            } catch (NullPointerException n) {
                try {
    ...
    public void drawBlockStroke(DrawSurface surface) {
        try {
            ...
            surface.drawLine((int) upperL.getX(), (int) upperL.getY(), (int) (upperL.getX() + this.rec.getWidth()),
                    (int) upperL.getY());

            // draw Horizontal line from down
            surface.drawLine((int) upperL.getX(), (int) (upperL.getY() - this.rec.getHeight()),
                    (int) (upperL.getX() + this.rec.getWidth()), (int) (upperL.getY() - this.rec.getHeight()));
    ...
    public Image getImage() {
        if (this.image == null) {
            System.out.println("Problem, image of the block is null");
        }
        return this.image;
    }

    /**
     * @return the stroke color of the block.
     */
    public Color getStroke() {
        if (this.blockStrokeColor == null) {
            System.out.println("Problem, stroke color of the block is null");
        }
        return this.blockStrokeColor;
    }

    /**
     * @param r - Rectangle to set.
     */
    public void setBlockRectangle(Rectangle r) {
        this.rec = r;
    }

    /**
     * set to hitted, using the rec of this block.
     */
    public void setRecToHitted() {
        this.rec.setToHitted();
    }

    /**
     * @return true if the block was hitted. Otherwise - false.
     */
    public boolean isHitted() {
        return this.rec.isHitted();
    }

    /**
     * set the block as a block of a Shield.
     */
    public void setAsShieldBlock() {
        this.rec.setAsShield();
    }

    /**
     * @return true if this block is a block of shield.
     * Otherwise, return false.
     */
    public boolean getIfShield() {
        return this.rec.ifShield();
    }


    /**
     * @param p - point to set to.
     * set (x,y) start (left upper) of the rectangle of the block.
     */
