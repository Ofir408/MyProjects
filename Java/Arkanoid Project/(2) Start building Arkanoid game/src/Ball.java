...
public class Ball implements Sprite {
    ...
    /**
     * @param newGameEnvironment - object to set as the current
     *                           game environment.
     */
    ...
    /**
     * The function computes the trajectory of the ball (his movement in
     * one step according dx, dy velocity.
     *
    ...
    /**
     * adding the ball and the block to the game,
     * with calling the appropriate game methods.
     *
     * @param g - the game object, that we will add to him
     *          the current ball.
     */
    @Override
    public void addToGame(final Game g) {
        g.addSprite(this);
    }

    /**
     * What should timePassed do?
    ...
    /*
     * helper function that checks if we need to change y.
     * if yes - return true
     * otherwise return false
     *
     * @param c - CollisionInfo object.
     * @return true if need to change y, otherwise false
     *
    private static boolean checkIfChangeY(final CollisionInfo c) {

        //double xC = c.collisionPoint().getX();
        double yC = c.collisionPoint().getY();
        // double width = c.collisionObject().
        //getCollisionRectangle().getWidth();
        double height = c.collisionObject().getCollisionRectangle().getHeight();
        Point recUpperLeft = c.collisionObject().
                getCollisionRectangle().getUpperLeft();

        if (yC == recUpperLeft.getY() || yC == recUpperLeft.getY() + height) {
            return true;
        }
        return false;
    } */
    ...
    /**
     * Update of The function moves one step.
     ...
     * ending where the velocity will take the ball if no collisions
     * will occur).
     * <p>
     ...
     * 2.2.2) move the ball to "almost" the hit point, but just slightly
     * before it.
     * 2.2.3) notify the hit object (using its hit() method) that a
     * collision occurred.
     * 2.2.4) update the velocity to the new velocity returned by
     * the hit() method.
     */
    public void moveOneStep() {
        CollisionInfo infoCollision = this.gameEnvironment.
                getClosestCollision(this.ballTrajectory());

        if (infoCollision == null) {
            // it means no collision in this movement step make the move
            // to the end of the current trajectory.
            this.centerPoint = this.ballVelocity.applyToPoint(
                    this.centerPoint);
            return;
        }
        ...
        this.setVelocity(infoCollision.collisionObject().hit(
                infoCollision.collisionPoint(),
                this.ballVelocity, this));

        // it means that there is a hit. 2.2 comment.

       /* if (collisionPoint.getX() < rec.getUpperLeft().
                * getX()) {
            // just -1 to x in this time
            this.centerPoint = new Point(infoCollision.
                    collisionPoint().
                    getX() - 1,
                    infoCollision.collisionPoint().
                            getY());
        } else if (collisionPoint.getY() < rec.getUpperLeft().getY()
                - rec.getHeight()) {
            // just -1 to y
            this.centerPoint = new Point(infoCollision.
                    collisionPoint().
                    getX(), infoCollision.collisionPoint().
                    getY() - 1);
        }
        ...
        } else if (collisionPoint.getX() > rec.getUpperLeft().getX()
                + rec.getWidth()) {
            // just +1 to x
            this.centerPoint = new Point(infoCollision.
                    collisionPoint().
                    getX() + 1, infoCollision
                    .collisionPoint().getY());
        } */
     ...
     *
     * @param c - is the intersection point between the ball
     *          and the other object (block, paddle..).
     */
    private void moveToClosePoint(final Point c) {
        double x = 0.01;
        if (this.ballVelocity.getDx() >= 0) {
            if (this.ballVelocity.getDy() >= 0) {
                this.centerPoint = new Point(c.getX() - x,
                        c.getY() - x);
            } else {
                this.centerPoint = new Point(c.getX() - x,
                        c.getY() + x);
