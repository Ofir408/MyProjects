...
public class GameEnvironment {
     ...
     *
     * @param c - Collidable object that we will remove him from the collection.
     */
    ...
    public CollisionInfo getClosestCollision(final Line trajectory) {
        ...
        List<Point> intersPointList;
        List<CollisionInfo> collisionInfoList = new ArrayList
                <CollisionInfo>();
        List<Collidable> collidableCollection = new ArrayList<Collidable>(this.collidableList);
        for (int i = 0; i < collidableCollection.size(); i += 1) {
            if (collidableCollection.get(i) instanceof Paddle) {
                Paddle current = (Paddle) collidableCollection.get(i);
                Rectangle r = current.getCollisionRectangle();
                Rectangle rect = new Rectangle(new Point(r.getUpperLeft().getX(),
                        r.getUpperLeft().getY() + r.getHeight()),
                        r.getWidth(), r.getHeight());
                intersPointList = rect.
                        intersectionPoints(trajectory);

            } else {
                intersPointList = collidableCollection.
