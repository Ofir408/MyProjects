...
/**
 * This program is a Paddle class.
 * Paddle is for the user, the tool that with this he
 * can to move and hit the balls in our game.
 *
...
public class Paddle implements Sprite, Collidable {
    ...
    /**
     * the speed of the paddle (just x so double and not
     * velocity object).
     */
    private double dx;
    /**
     ...
     */
    public Paddle(final KeyboardSensor keyB, final Rectangle rect,
                  final double xSpeed, final Color newColor) {
        this.keyboard = keyB;
        this.rec = rect;
        this.dx = xSpeed;
        this.color = newColor;
     ...
     * @param newColor - the color of the paddle.
     */
    public Paddle(final Rectangle rect,
                  final double xSpeed, final Color newColor) {
        this.rec = rect;
        this.dx = xSpeed;
        this.color = newColor;
     ...
     * Checking with if statements in order do not exit from our screen.
     */
    public void moveRight() {
        // minus 15 in order prevent going to the end.
        double screenWidth = 800 - 25;
        if (this.rec.getUpperLeft().getX() + this.rec.getWidth()
                + this.dx < screenWidth) {
            // it means that we can move right
            this.rec = new Rectangle(new Point(this.
                    rec.getUpperLeft().
                    getX() + this.dx, this.rec.
                    getUpperLeft().getY()),
                    this.rec.getWidth(),
                    this.rec.getHeight());
        } else {
            ...
            this.rec = new Rectangle(new Point(screenWidth
                    - this.rec.getWidth(),
    ...
    public void moveLeft() {
        // plus 10 in order prevent going to the end.
        ...
        if (currentX > 25 + this.dx) {
            ...
            this.rec = new Rectangle(new Point(currentX
                    - this.dx, currentY),
                    this.rec.getWidth(),
                    this.rec.getHeight());
        } else {
            ...
            // exit the screen
            this.rec = new Rectangle(new Point(25,
                    currentY),
                    this.rec.getWidth(),
                    this.rec.getHeight());
        }
    ...
    /**
     * How does the Paddle move? its timePassed method should
     ...
     * @param d - DrawSurface that helps us the drae the paddle
     * on the screen.
     */
    ...
    @Override
    /**
     * @param collisionPoint - the intersection point between the
     * paddle and the ball.
     * @param currentVelocity - the current velocity before the hit.
     * @return new Velocity after the hit, that his calculation depends
     * in which from the 5 equally-spaced regions it hits.
     */
    public Velocity hit(final Ball ball, final Point collisionPoint,
                        final Velocity currentVelocity) {

        int partNumber;
        double sizeEachPart = this.rec.getWidth() / 5.00;
        double temp = (collisionPoint.getX()
        ...
        if (currentVelocity.getDy() < 0) {
            ball.setCenterPoint(new Point(collisionPoint.getX() + ball.getVelocity().getDx(),
                    collisionPoint.getY()
                            - this.rec.getHeight() + ball.getVelocity().getDy()));
        }
        ...
        return Velocity.fromAngleAndSpeed((270 + 30 * partNumber)
                % 360, currentVelocity.getSpeed());
    }
     ...
     * @param g - the game to add the current paddle to him.
     */
