...
public class GameFlow {
    ...
    /**
     * @param newPath - String of the current path of symbols->messages->paths.
     */
    ...
    public void runLevels() {
        ...
        Map<String, String> symbolsAndMessages = SetsReaderOfLevels.getSymbolsAndMessages(this.getInputReader());
        for (String currentSymbol : symbolsAndMessages.keySet()) {
            List<LevelInformation> currentLevelsForSymbol = SetsReaderOfLevels.getLevelsOfSymbol(currentSymbol,
                    this.getInputReader());
            subMenu.addSelection(currentSymbol, symbolsAndMessages.get(currentSymbol),
                    new EasyLevels(currentLevelsForSymbol, this));
        }
    ...
    /**
     * @return InputStreamReader of this path.
     */
    private InputStreamReader getInputReader() {
        try {
            return new InputStreamReader(ClassLoader.getSystemClassLoader().
                    getResourceAsStream(this.path));
        } catch (Exception e) {
            System.out.println("can't find this path, Are you sure that the path is true? :) ");
            System.exit(0);
            return null;
        }
    }
